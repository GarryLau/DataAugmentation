...
namespace caffe {
	...
	void DataTransformer<Dtype>::Transform(const cv::Mat& img, Blob<Dtype>* transformed_blob)
	{
		...
		const bool do_resize_to_min_side_min = min_side_min > 0 && phase_ == TRAIN;
		const bool do_resize_to_min_side_max = min_side_max > 0 && phase_ == TRAIN;
		const bool do_resize_to_min_side = min_side > 0 && phase_ == TRAIN;
		const bool do_affine = affine_min_scale > 0 && affine_max_scale > 0 && affine_max_scale > affine_min_scale && phase_ == TRAIN;
		...
		if (do_affine)
		{
			...
			float affine_angle = 1.0 * Rand(rotation_angle * 2 + 1) - rotation_angle;
			float affine_scale = affine_min_scale + Rand((affine_max_scale - affine_min_scale) * 10) / 10.f;
		...
		if (do_resize_to_min_side)
		{
			...
			cv::resize(cv_img, cv_img, cv::Size(data_mean_.width(), data_mean_.height()));
		}
